...
Hooks own UI-facing loading/error state by wrapping TanStack Query. They never call Supabase. They call api functions as `queryFn` / `mutationFn`.

**Align with react-state:** **Never** add a new hand-rolled `useState` + `useEffect` fetch hook. Server data uses `useQuery` / `useMutation` with scoped keys from `@/lib/query`.
...
import { createLesson, fetchLessons } from '../api/lessonsApi'
import type { LessonFormValues } from '../types/lesson.types'

export function useLessons(topicId: string, institutionId: string) {
  const queryClient = useQueryClient()
  const lessonsKey = [
    ...queryKeys.institution.root(institutionId),
    'topics',
    topicId,
    'lessons',
  ] as const
  ...
  const lessonsQuery = useQuery({
    ...
    enabled: Boolean(topicId && institutionId),
  })

  const addLessonMutation = useMutation({
    mutationFn: (values: LessonFormValues) => createLesson(topicId, values),
    onSuccess: async () => {
  ...
  return {
    ...
    error: lessonsQuery.error instanceof Error ? lessonsQuery.error.message : null,
    addLesson: addLessonMutation.mutateAsync,
  }
}
```

> **Note:** Prefer adding named helpers on `queryKeys` in `src/lib/query/queryKeys.ts` when a key is reused. Never invent unscoped keys like `['lessons', topicId]`.
