...
# React State Guide — WQ

## What this covers

Defaults for **when to use hooks**, **how to use them correctly**, and **anti-patterns to avoid** in this repo.
Covers built-in React hooks, **TanStack Query** for server state, custom hooks, and form-specific hooks (`react-hook-form`, `zod`, `@hookform/resolvers`).

Use when building components that need **state**, **server data**, **side effects**, **performance optimization**, **form validation**, or **reusable logic**.

**MD first:** update this file (and `SKILL.md`) before introducing a new data-fetching convention in application code.
...
| Remote / server data (load, cache, share)    | `useQuery` / `useMutation` | Hand-rolled `useState`+`useEffect` fetch |
| Subscribe to DOM/storage/events; cleanup     | `useEffect`                | Mirroring props into state; fetching     |
...
| Multiple related state pieces                | `useReducer`               | Nested `useState` calls                  |
...
**Use when:** You need to **synchronize** with something outside React (DOM, localStorage, event listeners, timers) — **not** remote/server data.

**Don't use when:** You're fetching APIs (use `useQuery`), mirroring props into state, updating other state, or computing values (use `useMemo` or move logic before `return`).
...
// ❌ Fetching server data in useEffect — use TanStack Query instead
useEffect(() => {
  const controller = new AbortController()
  fetchUser(userId, { signal: controller.signal }).then(setData).catch(setError)
  return () => controller.abort()
}, [userId])
// Instead: useQuery({ queryKey: ['user', userId], queryFn: () => api.getUser(userId) })
...
- **Cleanup functions are critical** — unsubscribe, remove listeners, clear timers.
- **Don't put effects inside conditionals** — React must call all hooks in the same order on every render.
- **One concern per effect** — separate timers from event listeners; never mix in server fetches.
...
```tsx
// ✅ Server state: TanStack Query + feature API (scoped key, shared cache, invalidatable)
...
// ✅ Reusable, React-specific: subscribe to localStorage (client state — not server data)
...
// ❌ Hand-rolled fetch hook — private cache nobody else can invalidate
...
// Instead: wrap useQuery and call the feature API as queryFn (see useUser above).
// No new manual fetch hooks — existing ones migrate incrementally.
...
**Naming:** Always start with `use`, e.g., `useUser`, `useLocalStorage`, `useDebounce`. Prefer `useEntity` for queries and `useActionEntity` for mutations (e.g. `useUsers`, `useCreateUser`).
...
- **Query/mutation hooks** (feature only): `src/features/<domain>/hooks/useUser.ts` — `queryFn` calls `*Api.ts` only
...
| Hand-rolled fetch (`useState`+`useEffect`) | Private cache; no shared invalidation | `useQuery` / `useMutation`                       |
| Storing async/server data in `useState`    | Race conditions, manual loading state | TanStack Query                                   |
| Multiple `useEffect` calls for same data   | Hard to maintain, sync bugs           | One concern per effect; useQuery for server data |
...
```tsx
// ✅ Run once on mount (subscriptions / setup — not server fetch)
useEffect(() => {
  // subscribe or start a timer
}, [])

// ✅ Run when userId changes (still not for fetching — use useQuery for that)
useEffect(() => {
  // sync a non-server side effect tied to userId
}, [userId])
...
test('useUser loads user via query', async () => {
...
| `useQuery`        | Remote / server data                       | `useQuery({ queryKey, queryFn: () => api.get() })` |
| `useMutation`     | Remote writes; invalidate related queries  | `useMutation({ mutationFn: api.save })`            |
| `useEffect`       | DOM/storage/subscription sync (not fetch)  | `useEffect(() => { …; return cleanup }, [])`       |
...
| Custom hook       | Reusable React-specific logic              | `useUser` (wraps `useQuery`), `useLocalStorage`    |
...
1. **MD first.** Convention changes land in this skill before application code — agents read these files.
2. **Server data → `useQuery` / `useMutation`.** Never new hand-rolled fetch hooks.
3. **Client state → `useState` / `useLocalStorage` / editor state.** Keep canvas and Lexical out of Query.
4. **`useEffect` for sync only** (DOM, storage, subscriptions) — not fetching.
5. **Don't optimize prematurely.** Add `useCallback` or `useMemo` only if profiling shows a problem.
6. **Extract reusable logic into custom hooks** when the same React pattern repeats.
7. **Use form hooks for real forms.** Login, signup, settings — Zod validates, mutations submit.
8. **Trust the linter.** ESLint will tell you when you've missed a dependency.
