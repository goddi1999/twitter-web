...
✅ ERLAUBT im Audit:
  event_type:        cloud_file.created
                     ...
                     | cloud_file.restored          (Status-Wechsel 'deleted' → 'active')
...
### 4.10 `public.courses` / `public.games` (content lifecycle + Trash)

Emitted by `audit.log_courses_audit` / `audit.log_games_audit` (content audit + Trash purge).

```
✅ ERLAUBT im Audit:
  event_type (courses):
    course.created | course.deleted | course.restored | course.purged
    | course.published | course.unpublished
  event_type (games):
    game.created | game.deleted | game.restored | game.purged
    | game.archived | game.published | game.status_changed
  subject_id:        course / game UUID
  institution_id:    UUID (nullable on some legacy rows)
  payload (courses): is_published (boolean)
  payload (games):   old_status / new_status (enum codes only)
  metadata.visibility_level: 'institution_admin'
  metadata.context:  course_id | game_id

  Soft-delete (deleted_at NULL → set) → *.deleted
  Restore (deleted_at set → NULL) → *.restored
  Hard DELETE while deleted_at IS NOT NULL (Trash reaper) → *.purged
  Hard DELETE while still live → *.deleted

❌ VERBOTEN:
  title, description, message/body, Lexical/content JSONB
  Vollständige Zeilen-Dumps

HINWEIS Notes:
  public.notes bleiben owner-privat — kein audit.events-Trigger (persönliche Notizen).
```
